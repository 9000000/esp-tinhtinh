            ...
            while(remian_attr_len > 0) {
                ...
                switch (AttributeID)
                {
                    case NotificationAttributeIDAppIdentifier:
                        ...
                        // Identifier[len] = 0x00;
                        // memcpy(&Identifier, &attrs[3], len);
                        // ESP_LOGI(BLE_ANCS_TAG, "Identifier: %s", Identifier);
                        // ESP_LOG_BUFFER_CHAR("Identifier", &attrs[3], len);
                        len = len > sizeof(noti.identifier) ? sizeof(noti.identifier) : len;
                        memcpy(noti.identifier, &attrs[3], len);
                        noti.identifier[len] = 0x00;
                        break;
                    case NotificationAttributeIDTitle:
                        // ESP_LOG_BUFFER_CHAR("Title", &attrs[3], len);
                        // uint8_t Title[128];
                        // Title[len] = 0x00;
                        // memcpy(&Title, &attrs[3], len);
                        // ESP_LOGI(BLE_ANCS_TAG, "Title: %s", Title);
                        len = len > sizeof(noti.title) ? sizeof(noti.title) : len;
                        memcpy(noti.title, &attrs[3], len);
                        noti.title[len] = 0x00;
                        break;
                    ...
                    case NotificationAttributeIDMessage:
                        ...
                        // Message[len] = 0x00;
                        // memcpy(&Message, &attrs[3], len);
                        // ESP_LOGI(BLE_ANCS_TAG, "Message: %s", Message);
                        len = len > sizeof(noti.message) ? sizeof(noti.message) : len;
                        memcpy(noti.message, &attrs[3], len);
                        noti.message[len] = 0x00;
                        break;
