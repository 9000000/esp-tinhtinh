...
#define NVS_KEY_MAC "allowed_mac"

#define BLE_ANCS_TAG "KTM_MAIN"
#define EXAMPLE_DEVICE_NAME "Loa Tinh Tinh by KTM"
...
#define LED_CONNECTED_GPIO GPIO_NUM_22    // Chân nối LED, tùy chỉnh nếu bạn dùng GPIO khác
#define LED_DISCONNECTED_GPIO GPIO_NUM_23 // Chân nối LED, tùy chỉnh nếu bạn dùng GPIO khác
#define RESET_BUTTON_GPIO GPIO_NUM_0      // Chân nối nút nhấn, tùy chỉnh nếu bạn dùng GPIO khác
...
static uint8_t allowed_mac[6] = {0};
static bool mac_saved = false;

void save_mac_to_nvs(uint8_t *mac)
{
    nvs_handle_t nvs_handle;
    if (nvs_open(NVS_NAMESPACE, NVS_READWRITE, &nvs_handle) == ESP_OK)
    {
        nvs_set_blob(nvs_handle, NVS_KEY_MAC, mac, 6);
        nvs_commit(nvs_handle);
        nvs_close(nvs_handle);
    }
}

bool load_mac_from_nvs(uint8_t *mac)
{
    nvs_handle_t nvs_handle;
    size_t required_size = 6;
    if (nvs_open(NVS_NAMESPACE, NVS_READONLY, &nvs_handle) == ESP_OK)
    {
        if (nvs_get_blob(nvs_handle, NVS_KEY_MAC, mac, &required_size) == ESP_OK && required_size == 6)
        {
            nvs_close(nvs_handle);
            return true;
        }
...
void erase_mac_from_nvs()
{
    nvs_handle_t nvs_handle;
    if (nvs_open(NVS_NAMESPACE, NVS_READWRITE, &nvs_handle) == ESP_OK)
    {
        nvs_erase_key(nvs_handle, NVS_KEY_MAC);
        nvs_commit(nvs_handle);
        nvs_close(nvs_handle);
        ESP_LOGI(BLE_ANCS_TAG, "MAC erased from NVS");
    }
}
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT:
    {
        ...
        gpio_set_level(LED_DISCONNECTED_GPIO, 1);
    ...
    case ESP_GATTC_DISCONNECT_EVT:
        ...
        int32_t value = -2;
        xQueueSend(xQueue, &value, portMAX_DELAY);
        gpio_set_level(LED_CONNECTED_GPIO, 1);
        gpio_set_level(LED_DISCONNECTED_GPIO, 0);
    ...
    case ESP_GATTC_CONNECT_EVT:
        ...
        if (!mac_saved)
        {
            // Lần đầu tiên, lưu MAC vào NVS
            save_mac_to_nvs(param->connect.remote_bda);
            memcpy(allowed_mac, param->connect.remote_bda, 6);
            mac_saved = true;
            ESP_LOGI(BLE_ANCS_TAG, "Saved first device MAC to NVS");
        }
        else if (memcmp(param->connect.remote_bda, allowed_mac, 6) != 0)
        {
            // Nếu MAC không khớp, ngắt kết nối
            ESP_LOGW(BLE_ANCS_TAG, "Device not allowed, disconnecting!");
...
void reset_mac_task(void *arg)
{
    gpio_set_direction(RESET_BUTTON_GPIO, GPIO_MODE_INPUT);
    gpio_pullup_en(RESET_BUTTON_GPIO);
    ...
    while (1)
    {
        if (gpio_get_level(RESET_BUTTON_GPIO) == 0) // Button pressed (active low)
        {
            ...
            while (gpio_get_level(RESET_BUTTON_GPIO) == 0)
            {
                ...
                if ((esp_timer_get_time() - press_start) > 5000000) // 5 seconds
                {
                    erase_mac_from_nvs();
                    int32_t value = -4;
                    xQueueSend(xQueue, &value, portMAX_DELAY);
                    vTaskDelay(pdMS_TO_TICKS(4000));
                    esp_restart(); // Restart to allow new pairing
                }
            }
        }
...
void app_main(void)
{
    ...
    // Cấu hình GPIO ở chế độ output
    gpio_set_direction(LED_CONNECTED_GPIO, GPIO_MODE_OUTPUT);
    gpio_set_level(LED_CONNECTED_GPIO, 1);

    gpio_set_direction(LED_DISCONNECTED_GPIO, GPIO_MODE_OUTPUT);
    gpio_set_level(LED_DISCONNECTED_GPIO, 0);
    ...
    // Đọc MAC đã lưu (nếu có)
    mac_saved = load_mac_from_nvs(allowed_mac);
    ESP_LOGI(BLE_ANCS_TAG, "MAC saved: %s", mac_saved ? "true" : "false");
    if (mac_saved)
    {
        ESP_LOGI(BLE_ANCS_TAG, "Allowed MAC: %02X:%02X:%02X:%02X:%02X:%02X",
    ...
    sound_init();
    xTaskCreate(reset_mac_task, "reset_mac_task", 2048, NULL, 5, NULL);
}
